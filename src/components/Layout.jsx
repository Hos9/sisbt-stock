...
const GridIcon = iconWrap(<><rect x="3" y="3" width="7" height="7" rx="1.5" /><rect x="14" y="3" width="7" height="7" rx="1.5" /><rect x="3" y="14" width="7" height="7" rx="1.5" /><rect x="14" y="14" width="7" height="7" rx="1.5" /></>)
const BoxIcon = iconWrap(<><path d="M21 8L12 3 3 8v8l9 5 9-5V8z" /><path d="M3 8l9 5 9-5" /><path d="M12 13v8" /></>)
const ArrowDownIcon = iconWrap(<><path d="M12 4v16" /><path d="M6 14l6 6 6-6" /></>)
const ArrowUpIcon = iconWrap(<><path d="M12 20V4" /><path d="M6 10l6-6 6 6" /></>)
const LayersIcon = iconWrap(<><path d="M12 3l9 5-9 5-9-5 9-5z" /><path d="M3 13l9 5 9-5" /></>)
const HistoryIcon = iconWrap(<><path d="M3 12a9 9 0 109-9" /><path d="M3 4v5h5" /><path d="M12 7v5l4 2" /></>)
const TagIcon = iconWrap(<><path d="M20.5 12.3l-8-8H4v8.5l8 8a1.5 1.5 0 002 0l6.5-6.5a1.5 1.5 0 000-2z" /><circle cx="8.5" cy="8.5" r="1.2" /></>)
const ChartIcon = iconWrap(<><path d="M4 20V10" /><path d="M12 20V4" /><path d="M20 20v-7" /></>)
const CogIcon = iconWrap(<><circle cx="12" cy="12" r="3" /><path d="M19.4 15a1.7 1.7 0 00.3 1.9l.1.1a2 2 0 11-2.9 2.9l-.1-.1a1.7 1.7 0 00-1.9-.3 1.7 1.7 0 00-1 1.5V21a2 2 0 11-4 0v-.1a1.7 1.7 0 00-1-1.6 1.7 1.7 0 00-1.9.3l-.1.1a2 2 0 11-2.9-2.9l.1-.1a1.7 1.7 0 00.3-1.9 1.7 1.7 0 00-1.5-1H3a2 2 0 110-4h.1a1.7 1.7 0 001.5-1 1.7 1.7 0 00-.3-1.9l-.1-.1a2 2 0 112.9-2.9l.1.1a1.7 1.7 0 001.9.3H9a1.7 1.7 0 001-1.5V3a2 2 0 114 0v.1a1.7 1.7 0 001 1.5 1.7 1.7 0 001.9-.3l.1-.1a2 2 0 112.9 2.9l-.1.1a1.7 1.7 0 00-.3 1.9V9a1.7 1.7 0 001.5 1H21a2 2 0 110 4h-.1a1.7 1.7 0 00-1.5 1z" /></>)
const LogoutIcon = iconWrap(<><path d="M9 21H5a2 2 0 01-2-2V5a2 2 0 012-2h4" /><path d="M16 17l5-5-5-5" /><path d="M21 12H9" /></>)
const MenuIcon = iconWrap(<><path d="M4 6h16" /><path d="M4 12h16" /><path d="M4 18h16" /></>)
    ...
    <div className="min-h-screen bg-surface-muted">
      ...
      >
        <div className="flex h-16 items-center gap-2 px-6 border-b border-white/10">
          ...
          <span className="font-display font-bold text-lg tracking-tight">NetStock</span>
        </div>
        ...
        <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-surface-border bg-white/90 px-4 backdrop-blur sm:px-6">
          ...
          <div className="flex items-center gap-3">
            <div className="hidden text-right sm:block">
              <p className="text-sm font-medium text-ink-900">{profile?.full_name || user?.email}</p>
              <p className="text-xs capitalize text-ink-700/60">{profile?.role || (isAdmin ? 'admin' : 'staff')}</p>
            </div>
